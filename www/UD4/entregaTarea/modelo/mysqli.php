...
function modificar_tabla_usuarios(){
    try{
        $conexion = conectaTareas();

        $sql = "ALTER TABLE usuarios ADD rol INT NOT NULL DEFAULT 0";
        if ($conexion->query($sql)){
            return [true, 'Tabla "usuarios" modificada correctamente'];
        }else
        {
            return [false, 'No se pudo modificar la tabla "usuarios".'];
        }
...
function ficheros(){
    try {
        ...
        if ($conexion->connect_error){
            ...
            $sql = 'CREATE TABLE `ficheros` (`id` INT NOT NULL AUTO_INCREMENT, `nombre` VARCHAR(100) NOT NULL, `file` VARCHAR(250) NOT NULL, `descripcion` VARCHAR(250) , `id_tarea` INT NOT NULL, PRIMARY KEY (`id`), FOREIGN KEY (`id_tarea`) REFERENCES `tareas`(`id`))';
            if ($conexion->query($sql)){
